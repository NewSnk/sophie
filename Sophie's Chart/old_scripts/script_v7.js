...
const btn4 = d3.select("#adjustment");
const btn5 = d3.select("#deficit");
...
d3.csv("./data/U.S.TextileFiberTrade_with10YrAvg.csv", parse).then(function (data) {
    ...
    const adjustment = ["value_net_above", "value_below_adj"]
    const deficit = ["value_below_adj"]

    const raw_colors = ["#03a1fc"] //dark green
    const below_colors = ["#a9c1d4", "#fc0318"] // green and red
    const pre_adjustment_colors = ["#a9c1d4", "#62bd74", "#fc0318"]
    const adjustment_colors = ["#a9c1d4", "#fc0318"]
    const adjustment_colors_for_drop = ["#ffffff", "#fc0318"]
    ...
    function draw(dataset, selected_key, selected_color, series) {
        ...
        let path = svg.append("path").attr("class", series)
    ...
    //this function handles the data-driven elements
    function draw_final(dataset, selected_color, series) {
        ...
        const colorScale1 = d3.scaleOrdinal()
            .domain(adjustment)
            .range(adjustment_colors_for_drop)

        const colorScale2 = d3.scaleOrdinal()
            .domain(deficit)
            .range(deficit_colors)

        let stackedData1 = stack1(dataset);
        let stackedData2 = stack2(dataset);
    ...
    btn3.on("click", function () {
        draw(cotton_only, pre_adjustment, pre_adjustment_colors, series3);
        d3.selectAll(".series1").remove();
    ...
    btn4.on("click", function () {
        draw(cotton_only, adjustment, adjustment_colors, series4);
        d3.selectAll(".series1").remove();
        d3.selectAll(".series2").remove();
    ...
    btn5.on("click", function () {
        ...
        draw_final(cotton_only, deficit, deficit_colors, series5);
    });
